...
//	int acceptPrice <- rnd(0, 100);
	...
	// For vickrey, the max accepted price should be the maxLosePremium,
	// because the max lose money that one risk should be the max Lose premium
	int marketPrice <- 100;
	...
	reflex receivePrice when: (!empty(cfps)) and location distance_to arena < 16 and joinAuction
	{
		...
		if (price <= maxLosePremium)
		{
			price <- price + int((maxLosePremium-price) / 2);
